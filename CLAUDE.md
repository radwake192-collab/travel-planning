...
precedence over asking the user questions you can answer yourself:

- **`travel-profile.md`** — home airport, travelers, loyalty balances, card benefits, and
  standing preferences. **Read it before any trip search.** Fields marked `TODO` are
  genuinely unknown and are fair to ask about; everything else is already answered, so
  don't re-ask it.
- **`trips/`** — trip briefs with dates, open questions, and a per-trip search plan.
  Check for an existing brief before planning a trip; if one exists, continue it rather
  than starting over. Update the brief's `status` and `booked_option` as things progress.
  See `trips/README.md` for the layout.
